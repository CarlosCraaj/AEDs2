...
class Musica
{
    ...
    */
    ...
    public static void PegarArquivo_2(String[] id, int n)
    {
        String tmp = "";
        String auxiliar = "";
        String[] pegarId;
...
class NoAN{
    ...
    public NoAN (Musica music){
        this(music, false, null, null);
    }
    public NoAN (Musica music, boolean cor){
        this(music, cor, null, null);
    }
    public NoAN (Musica music, boolean cor, NoAN esq, NoAN dir){
        this.cor = cor;
...
class Alvinegra {
	...
	private boolean pesquisar(String elemento, NoAN i) 
    {
        ...
        } else if (elemento.compareTo(i.music.getID()) == 0) {
            resp = true;
	...
	public void inserir(Musica musica) throws Exception 
    {
        ...
        if(raiz == null)
        {
            raiz = new NoAN(musica);
        } else if (raiz.esq == null && raiz.dir == null)
        {
            if (musica.getID().compareTo(raiz.music.getID()) < 0)
            {
                raiz.esq = new NoAN(musica);
            } else 
            {
                raiz.dir = new NoAN(musica);
            }
        ...
        } else if (raiz.esq == null)
        {
            ...
            {
                raiz.esq = new NoAN(musica);
            } else if (musica.getID().compareTo(raiz.dir.music.getID()) < 0)
            {
                raiz.esq = new NoAN(raiz.music);
                raiz.music = musica;
            } else 
            {
                raiz.esq = new NoAN(raiz.music);
                raiz.music = raiz.dir.music;
                raiz.dir.music = musica;
            }
        ...
        } else if (raiz.dir == null)
        {
            ...
            {
                raiz.dir = new NoAN(musica);
            } else if (musica.getID().compareTo(raiz.esq.music.getID()) > 0)
            {
                raiz.dir = new NoAN(raiz.music);
                raiz.music = musica;
            } else 
            {
                raiz.dir = new NoAN(raiz.music);
                raiz.music = raiz.esq.music;
                raiz.esq.music = musica;
            }
        ...
        } else 
        {
            ...
            inserir(musica, null, null, null, raiz);
        }
    ...
    private void balancear(NoAN bisavo, NoAN avo, NoAN pai, NoAN i)
    {
        ...
        if(pai.cor == true)
        {
            if(pai.music.getID().compareTo(avo.music.getID()) > 0)
            {
                if(i.music.getID().compareTo(pai.music.getID()) > 0)
                {
                avo = rotacaoEsq(avo);
                } else 
                {
            ...
            {
                raiz = avo;
            } else if(avo.music.getID().compareTo(bisavo.music.getID()) < 0){
                bisavo.esq = avo;
            } else {
                bisavo.dir = avo;
            }
	...
	private void inserir(Musica musica, NoAN bisavo, NoAN avo, NoAN pai, NoAN i) throws Exception {
		if (i == null) 
        {
            if(musica.getID().compareTo(pai.music.getID()) < 0)
            {
                i = pai.esq = new NoAN(musica, true);
            } else 
            {
                i = pai.dir = new NoAN(musica, true);
            }
        ...
        } else 
        {
            ...
            if (musica.getID().compareTo(i.music.getID()) < 0) 
            {
                inserir(musica, avo, pai, i, i.esq);
            } else if (musica.getID().compareTo(i.music.getID()) > 0) 
            {
                inserir(musica, avo, pai, i, i.dir);
            } else 
            {
...
{
    public static Alvinegra tree = new Alvinegra();
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        do
        {
            AuxPes = MyIO.readLine();
            if(AuxPes.compareTo("FIM") != 0)
            {
                MyIO.println(AuxPes);
                String resp = (tree.pesquisar(AuxPes)) ? "SIM" : "NAO";
                MyIO.println(resp);
            }
        }while(Musica.isFim(AuxPes) == false);
