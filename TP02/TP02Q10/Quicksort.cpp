...
void quicksort(int esq, int dir, int comp[], int mov[]) {
    int i = esq, j = dir;
    int pivo;
    Musica *tmp;
    pivo = musics[(dir+esq)/2]->duration_ms;
    while (i <= j) {
        while (musics[i]->duration_ms < pivo) 
        {
            i++;
            comp[0] += 1;
        }
        ...
        while (musics[j]->duration_ms > pivo) 
        {
            j--;
            comp[0] += 1;
        }
    ...
    if (esq < j)  quicksort(esq, j, comp, mov);
    if (i < dir)  quicksort(i, dir, comp, mov);
}
...
int main()
{
    ...
    clock_t inicio, fim;
    double total;
    FILE* file = fopen("683471_quicksort.txt", "w");
    inicio = clock();
    quicksort(0, len-1, comp, mov);
    fim = clock();
