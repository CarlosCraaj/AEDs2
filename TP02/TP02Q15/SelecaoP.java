...
class SelecaoP
{
    ...
    static int k = 10;
    public static void SelecaoParcial(Musica[] musics, int n, int[] comp, int[] mov)
    {
        ...
        for(int i = 0; i < k; i++)
        {
            ...
            {
                comp[0] += 1;
                if(musics[menor].getNome().compareTo(musics[j].getNome()) > 0)
                {
                    menor = j;
                }
            }
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        comp[0] = 0; mov[0] = 0;
        Arq.openWrite("683471_selecaoParcial.txt", "UTF-8");
        inicio = algoritmo.now();
        SelecaoParcial(musics, n, comp, mov);
        fim = algoritmo.now();
