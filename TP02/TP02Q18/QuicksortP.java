...
class QuicksortP
{
    ...
    public static void QuicksortParcial(int esq, int dir, int[] comp, int[] mov) 
    {
        Musica tmp = new Musica();
        int i = esq;
        int j = dir;
        int pivo = musics[(esq+dir)/2].getDuration_ms();
        while(i <= j)
        {
            while (musics[i].getDuration_ms() < pivo) 
            {
                i++;
                comp[0] += 1;
            }
            ...
            while (musics[j].getDuration_ms() > pivo) 
            {
                j--;
                comp[0] += 1;
            }
        ...
        if (esq < j)  QuicksortParcial(esq, j, comp, mov);
        if (i < k && i < dir)  QuicksortParcial(i, dir, comp, mov);     
    }
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        comp[0] = 0; mov[0] = 0;
        Arq.openWrite("683471_quicksortParcial.txt", "UTF-8");
        inicio = algoritmo.now();
        QuicksortParcial(0, n-1, comp, mov);
        fim = algoritmo.now();
