...
class CountingS
{
    ...
    public static void CountingSort(int n, int[] comp, int[] mov) 
    {
        //Array para contar o numero de ocorrencias de cada elemento
        Musica[] count = new Musica[getMaior(n) + 1];
        Musica[] ordenado = new Musica[n];

        //Inicializar cada posicao do array de contagem 
        for (int i = 0; i < count.length; count[i] = new Musica(), count[i].setPopularity(0), i++);

        //Inicializar cada posicao do array "ordenado"
        for(int i = 0; i < n; ordenado[i] = musics[i], i++);

        //Agora, o count[i] contem o numero de elemento iguais a i
        for (int i = 0; i < n; count[musics[i].getPopularity()].setPopularity(1), i++);

        //Agora, o count[i] contem o numero de elemento menores ou iguais a i
        for(int i = 1; i < count.length; count[i].setPopularity(count[i-1].getPopularity()), i++);

        //Ordenando
        MyIO.println(n + "\t" + count.length + "\t" + ordenado.length);
        for(int i = n-1; i >= 0; ordenado[count[musics[i].getPopularity()-1].getPopularity()].setPopularity(musics[i].getPopularity()), count[musics[i].getPopularity()].setPopularity(-1), i--);

        //Copiando para o array original
        for(int i = 0; i < n; musics[i] = ordenado[i], i++);
    }

	public static int getMaior(int n) 
    {
	    int maior = musics[0].getPopularity();

		for (int i = 0; i < n; i++) 
        {
            if(maior < musics[i].getPopularity())
            {
                maior = musics[i].getPopularity();
            }
		}
	    return maior;	
	}
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        comp[0] = 0; mov[0] = 0;
        Arq.openWrite("683471_countingsort.txt", "UTF-8");
        inicio = algoritmo.now();
        CountingSort(n, comp, mov);
        fim = algoritmo.now();
