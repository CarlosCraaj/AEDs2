...
//Metodo baseado do arquivo shellsort.c da pasta fonte
void insercaoPorCor(int len, int cor, int h, int comp[], int mov[]){
    Musica *tmp;
    for (int i = (h + cor); i < len; i+=h) {
        tmp = musics[i];
        int j = i - h;
        while ((j >= 0) && (strcmp(musics[j]->id, tmp->id) > 0)) {
            musics[j + h] = musics[j];
            j-=h;
            comp[0] += 1;
            mov[0] += 1;
...
//Metodo baseado do arquivo shellsort.c da pasta fonte
void shellsort(int len, int comp[], int mov[]) {
    int h = 1;
    ...
    do {
        ...
        for(int cor = 0; cor < h; cor++){
            insercaoPorCor(len, cor, h, comp, mov);
        }
    } while (h != 1);
}
...
int main()
{
    ...
    clock_t inicio, fim;
    double total;
    FILE* file = fopen("683471_shellsort.txt", "w");
    inicio = clock();
    shellsort(len, comp, mov);
    fim = clock();
