...
void BubbleSort(int n, int comp[], int mov[]) 
{
    Musica *tmp;
    int i, j;
    for (i = (n - 1); i > 0; i--) 
    {
        for (j = 0; j < i; j++) 
        {
            if (musics[j]->danceability > musics[j + 1]->danceability)
            {
                tmp = musics[j];
                musics[j] = musics[j + 1];
                musics[j + 1] = tmp;
...
int main()
{
    ...
    clock_t inicio, fim;
    double total;
    FILE* file = fopen("683471_bolha.txt", "w");
    inicio = clock();
    BubbleSort(len, comp, mov);
    fim = clock();
