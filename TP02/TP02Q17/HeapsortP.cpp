...
typedef struct {
    ...
    char **list; int k;
    char *data; int dia; int mes; int ano;
...
int compararData(int i, int j)
{
    int resp = 0;
    if(musics[i]->ano > musics[j]->ano)
    {
        resp = 1;
    }else if(musics[i]->ano == musics[j]->ano)
    {
        if(musics[i]->mes > musics[j]->mes)
        {
            resp = 1;
        }else if(musics[i]->mes == musics[j]->mes)
        {
            if(musics[i]->dia > musics[j]->dia)
            {
                resp = 1;
            }else if(musics[i]->dia < musics[j]->dia)
            {
                resp = -1;
            }
        }else if(musics[i]->mes < musics[j]->mes)
...
void setData(int i, char* aux)
{
    int len = strlen(aux);
    int x = 0;
    char *dia = (char*)calloc(sizeof(char), 2); 
    char *mes = (char*)calloc(sizeof(char), 2); 
    char *ano = (char*)calloc(sizeof(char), 4);
    if(len <= 4)
    {
        for(int j = 0; j < 10; j++)
        {
    ...
    }else
    {
        for (int j = 0; j < len; j++)
        {
            ...
            if(j == 3)
            {
                ...
                mes[0] = musics[i]->data[j];
            }
            if(j == 4)
            {
                musics[i]->data[j] = aux[6];
                mes[1] = musics[i]->data[j];
            }
            if(j == 5)
            {
                musics[i]->data[j] = '/';
            ...
            if(j == 6)
            {
                ...
                ano[0] = musics[i]->data[j];
            }
            if(j == 7)
            {
                musics[i]->data[j] = aux[1];
                ano[1] = musics[i]->data[j];
            }
            if(j == 8)
            {
                musics[i]->data[j] = aux[2];
                ano[2] = musics[i]->data[j];
            }
            if(j == 9)
            {
                musics[i]->data[j] = aux[3];
                ano[3] = musics[i]->data[j];
            }
        }   
        musics[i]->dia = atoi(dia);
        musics[i]->mes = atoi(mes);
        musics[i]->ano = atoi(ano);
    }
    //printf("%d/%d/%d\n", musics[i]->dia, musics[i]->mes, musics[i]->ano);
}
...
void construir(int tam, int comp[], int mov[])
{
    Musica *tmp;
    for (int i = tam; i > 1 && compararData(i, i/2) == 1; i /= 2)
    {
        tmp = musics[i];
        musics[i] = musics[i/2];
        musics[i/2] = tmp;
...
int getMaiorFilho(int i, int tam, int comp[])
{
    int filho = 0;
    if(2*i == tam || compararData(2*i, 2*i + 1) == 1)
    {
        filho = 2*i;
    }else
    {
...
void reconstruir(int tam, int comp[], int mov[])
{
    int i = 1;
    while(i <= (tam/2))
    {
        Musica *aux;
        int filho = getMaiorFilho(i, tam, comp);
        if(compararData(i, filho))
        {
            aux = musics[i];
            musics[i] = musics[filho];
            musics[filho] = aux;
            mov[0] += 3;
            i = filho;
        }else
...
void HeapsortParcial(int k, int len, int comp[], int mov[])
{
    //Construção do heap
    for(int tam = 2; tam <= k; tam++)
    {
        construir(tam, comp, mov);
    }
    for(int i = k + 1; i <= len; i++)
    {
        //printf("%s - %s\n", musics[i]->data, musics[1]->data);
        if(compararData(i, 1) == -1)
        {
            Musica *aux;
            aux = musics[i];
            musics[i] = musics[1];
            musics[1] = aux;
            reconstruir(k, comp, mov);
        }
    }
    ...
    while(tamHeap > 1)
    {
        Musica *aux;
        aux = musics[1];
        musics[1] = musics[tamHeap];
        musics[tamHeap] = aux;
        mov[0] += 3;
        tamHeap--;
        reconstruir(tamHeap, comp, mov);
    }
}
...
int main()
{
    ...
    clock_t inicio, fim;
    double total;
    FILE* file = fopen("683471_heapsortParcial.txt", "w");
    inicio = clock();
    HeapsortParcial(10, len-1, comp, mov);
    fim = clock();
