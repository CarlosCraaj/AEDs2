...
class Mergesort
{
    ...
    public static void OrdenacaoMergesort(int esq, int dir, int[] comp, int[] mov) 
    {
        if(esq < dir)
        {
            int meio = (esq + dir)/2;
            OrdenacaoMergesort(esq, meio, comp, mov);
            OrdenacaoMergesort(meio + 1, dir, comp, mov);
    ...
    public static void intercalar(int esq, int meio, int dir, int[] comp, int[] mov)
    {
        ...
        {
            a2[j] = new Musica();
            a2[j] = musics[meio+j+1];
        }
        a2[j] = new Musica();

        //Sentinela no final dos dois arrays
        a1[i].setEnergy(999.0);
        a2[j].setEnergy(999.0);

        //Intercalacao propriamente dita
        for(i = j = 0, k = esq; k <= dir; k++)
        {
            musics[k] = (a1[i].getEnergy() <= a2[j].getEnergy()) ? a1[i++] : a2[j++];
            comp[0] += 1;
            mov[0] += 2;
        }
    }
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        comp[0] = 0; mov[0] = 0;
        Arq.openWrite("683471_mergesort.txt", "UTF-8");
        inicio = algoritmo.now();
        OrdenacaoMergesort(0, n-1, comp, mov);
        fim = algoritmo.now();
