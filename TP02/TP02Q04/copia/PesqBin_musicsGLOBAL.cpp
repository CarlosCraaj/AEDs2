...
void imprimir(int i)
{
    ...
    tmp[0] = '[';
    int j = 0;
    for(int i = 0; i < musics[i]->k; i++)
    {
        if(i == musics[i]->k-1)
        {
            strcat(tmp, musics[i]->list[i]);
        }else
        {
            strcat(tmp, musics[i]->list[i]);
            strcat(tmp, ",");
...
void OrdenarArrayMusica(int len)
{
    Musica *tmp;
    for(int i = 0; i < len; i++)
    {
        ...
        {
            //if(strcmp(musics[menor]->id, musics[j]->id) == 1)
            if(comparison(musics[menor]->id, musics[j]->id) == 1)
            {
                menor = j;
            }
            tmp = musics[menor];
...
void pesquisaBinaria(char *id, int len, char* resp, int* comp)
{
    strcpy(resp, "NAO");
    ...
    {
        meio = (esq + dir)/2;
        if(strcmp(id, musics[meio]->id) == 0)
        {
            comp[0] += 1;
            strcpy(resp, "SIM");
            esq = len;
        }else if(strcmp(id, musics[meio]->id) > 0)
        {
            comp[0] += 2;
            esq = meio + 1;
        }else
...
int main()
{
    ...
    while(i < nPes)
    {
        pesquisaBinaria(pubinPes[i], len, resp, comp);
        printf("%s\n", resp);
        i++;
    }
    fim = clock();
