...
class Heapsort
{
    static Musica[] musics = new Musica[310]; //Declara 310 objetos do tipo Musica

    //Posicao 0 = ano, 1 = mes e 2 = dia.
    public static int compararData(int[] data, int[] data1)
    {
        int resp = 0;
        //MyIO.println(data[0] + "/" + data[1] + "/" + data[2] + "\t\t" + data1[0] + "/" + data1[1] + "/" + data1[2]);
        if(data[0] > data1[0])
        {
            resp = 1;
    ...
    //Metodo baseado no metodo que esta na pasta fontes/u04
    public static void OrdenacaoHeapsort(int n, int[] comp, int[] mov) 
    {
    ...
    public static void construir(int tamHeap, int[] comp, int[] mov)
    {
        Musica tmp = new Musica();
        for(int i = tamHeap; i > 1 && compararData(musics[i].date.dataForCmp(), musics[i/2].date.dataForCmp()) == 1; i /= 2)
        {
            tmp = musics[i];
            musics[i] = musics[i/2];
            musics[i/2] = tmp;
    ...
    public static void reconstruir(int tamHeap, int[] comp, int[] mov)
    {
        int i = 1;
        Musica tmp = new Musica();
        while(i <= (tamHeap/2))
        {
            int filho = getMaiorFilho(i, tamHeap, comp);
            if(compararData(musics[i].date.dataForCmp(), musics[filho].date.dataForCmp()) == -1 )
            {
                tmp = musics[i];
    ...
    public static int getMaiorFilho(int i, int tamHeap, int[] comp)
    {
        int filho;
        if (2*i == tamHeap || compararData(musics[2*i].date.dataForCmp(), musics[2*i+1].date.dataForCmp()) == 1)
        {
            filho = 2*i;
        } else 
        {
    ...
    public static void main(String[] args) throws Exception
    {
        ...
        Musica algoritmo = new Musica();
        double inicio, fim;
        int n = 0;
        int[] comp = new int[1]; int[] mov = new int[1];
        ...
        do
        {
            ...
            n++;
        }while(Musica.isFim(pubin[n-1]) == false);
        n--; //Desconsiderar FIM

        Musica.PegarArquivo(pubin, n);
        ...
        comp[0] = 0; mov[0] = 0;
        Arq.openWrite("683471_heapsort.txt", "UTF-8");
        inicio = algoritmo.now();
        OrdenacaoHeapsort(n, comp, mov);
        fim = algoritmo.now();
        Arq.println("683471" + "\t" + comp[0] + "\t" + mov[0] + "\t" + (fim-inicio)/1000.0);
        Arq.close();
