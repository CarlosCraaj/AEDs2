...
class Lista
{
    ...
    public void inserirInicio(Musica musica) throws Exception {
        ...
        //levar elementos para o fim do musics
        for(int i = n; i > 0; i--){
            musics[i] = musics[i-1];
        }
    ...
    public void inserirFinal(Musica musica) throws Exception {
    ...
    public void inserir(Musica musica, int pos) throws Exception {

        //validar insercao
        if(n >= musics.length || pos < 0 || pos > n){
            throw new Exception("Erro ao inserir!");
        }
