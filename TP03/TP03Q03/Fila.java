...
class Fila
{
    ...
    public void inserir(Musica musica) throws Exception {
        ...
        if (((ultimo + 1) % musics.length) == primeiro) 
        {
            remover();
        }

        musics[ultimo] = musica;
        ultimo = (ultimo + 1) % musics.length;
        mostrarMedia();
    }
    ...
    public Musica remover() throws Exception {
        ...
        Musica resp = musics[primeiro];
        primeiro = (primeiro + 1) % musics.length;
        return resp;
    }

    public void mostrarMedia (){
        int soma = 0;
        double media = 0; double auxiliar = 0;
        int n = 0;
        for(int i = primeiro; i != ultimo; i = ((i + 1) % musics.length)) {
            soma += musics[i].getDuration_ms();
            n++;
        }

        media = (double)soma/n;
        auxiliar = media - (int)media;
        media = (auxiliar >= 0.5) ? media += 0.5 : media;
